...
function Signup({ display = "flex", formSwitcher, setDisplayLoginPopup, res }) {
  ...
  return (
    !showOtp ?
    <div className={`fixed py-10 top-0 left-0 h-full w-full justify-center bg-bgPrimary border-3 border-transparent backdrop-blur-md z-999999 p-6 overflow-y-auto vs ${display} `}>
      ...
      <section className={`flex flex-col max-w-[500px] w-[calc(100%-20px)] ${error ? "h-[555px]" : "h-[500px]"} m-auto `}>

        <div className={`w-full bg-white px-5 pt-2 pb-5 shadow-lg rounded m-auto ${error ? "h-[520px]" : "h-[510px]"} m-auto`}>
          <form className="flex flex-col w-full gap-4">
            ...
            <div className="centerMe">
              <button type="button" className="bg-bgPrimary rounded px-8 py-2 text-white" onClick={signupSubmit}  >
                Submit
              </button>
            </div>
