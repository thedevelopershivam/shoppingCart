...
function UserDropdownBtn({ href = "#", icon, title }) {
    return (
        <Link href={href} className='flex items-center gap-2 duration-300 hover:pl-2 text-gray-600 font-normal '>
            {icon && <span> {icon} </span>}
        ...
        <>
            <Wrapper className=' mx-auto w-full h-[calc(50px+1.5vw)] bg-bgPrimary flex justify-between items-center px-[calc(20px+2vw)] z9999'>
                ...
                <div className='hidden sm:flex gap-8'>
                    {user && !user?.email &&
                        <span className='font-semibold text-white cursor-pointer' onClick={() => dispatch(popUp("login"))}>
                            Login
                        </span>
                    }
                    ...
                    >
                        <UserDropdownBtn icon={<IoMdHeartEmpty size={20} />} title="Wishlist" />
                        <UserDropdownBtn icon={<FaBoxArchive />} title="Order" />
                        <UserDropdownBtn icon={<IoNotificationsOutline size={20} />} title="Notification" />
